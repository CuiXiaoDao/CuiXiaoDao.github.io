...
![](assets/2024-09-09-从一次HTTP请求看TCP%20IP协议/image-20240917162558331.png)
...
我们在PT(Packet Tracer)中构建以下拓扑的网络结构。

![](assets/2024-09-09-从一次HTTP请求看TCP%20IP协议/image-20241003205931794.png)
...
![](assets/2024-09-09-从一次HTTP请求看TCP%20IP协议/image-20241004120504866.png)
![](assets/2024-09-09-从一次HTTP请求看TCP%20IP协议/image-20241004122724065.png)
![](assets/2024-09-09-从一次HTTP请求看TCP%20IP协议/image-20241004122826414.png)





打开simulation, event filter中只保留我们关注的event类型，然后打开PC0的web browser，访问test.srv, 如下所示

![](assets/2024-09-09-从一次HTTP请求看TCP%20IP协议/image-20241003210707519.png)
![](assets/2024-09-09-从一次HTTP请求看TCP%20IP协议/image-20241003210732615.png)
...
### DNS查询
首先PC0拿到域名test.srv,会通过DNS协议向DNS sever查找该域名的IP。

由于DNS sever的IP（209.165.200.225）与PC0不在同一子网，DNS查询请求下一跳的地址被设为default gateway (也即网路拓扑中的router)的IP，192.168.0.1。如果PC0还没有记录router的MAC地址，会通过ARP协议查询router的MAC地址，收到router的回复后，会记录router的MAC地址，并将DNS查询请求在链路层的目标MAC地址为router的MAC地址。

接着PC0发出DNS查询请求，router收到DNS查询请求。数据包的目标IP地址是209.165.200.225，router根据路由表发现DNS server是与router直接相连的，因此数据包的下一跳地址被设为DNS sever的IP（209.165.200.225）。此时，路由器会做两件事
- 通过ARP协议查询DNS sever的MAC地址
- 数据包将由内网发到外网，路由器会进行地址转换，并将内网地址与端口的映射信息维护在NAT表

DNS server收到查询请求并回复该域名对应的IP (209.165.200.225)

![](assets/2024-09-09-从一次HTTP请求看TCP%20IP协议/image-20241004002514927.png)


### 建立TCP连接

PC0收到DNS server回复后，即知道test.srv的IP地址是209.165.200.225，在发送HTTP请求到server前，PC0首先会请求server建立TCP连接, 就这是经典的三次握手过程。

这个过程在网上有详细的阐述，这里引用 [网上的材料](https://www.9tut.com/tcp-and-udp-tutorial)


![](assets/2024-09-09-从一次HTTP请求看TCP%20IP协议/image-20241004003742712.png)
...
建立TCP连接后，PC0就会发送一个HTTP请求到test.srv，Src Port 1025是PC0分配给web browser的端口，web browser通过监听这个端口接收HTTP响应。
...
HTTP请求各层信息如下
![](assets/2024-09-09-从一次HTTP请求看TCP%20IP协议/image-20241004102317567.png)

该HTTP请求路径为PC0 -> router -> cable modem0 -> cloud0 -> test.srv，路由寻址类似前面DNS查询，主要包括路由查询、内外网地址转换，不再赘述。

### 响应HTTP请求
test.srv收到HTTP请求后，数据经过物理层、链路层、网络层和传输层，在传输层由TCP组装所有数据（请求数据在传输过程中可能被分片），发送到应用层，端口为80。HTTP server监听80端口，收到HTTP请求后，会准备响应数据，并将其发送回client (PC0)。
对于HTTP响应数据包，其
- 目标IP：即为请求数据包的源IP，也是路由器的外网地址
- 目标端口：即为请求数据包的源端口，也是路由器在地址转换时分配的端口，1025（NAT）

![](assets/2024-09-09-从一次HTTP请求看TCP%20IP协议/image-20241004110440676.png)
...
PC0的web browser监听1025端口接受到HTTP响应后，显示页面。
![](assets/2024-09-09-从一次HTTP请求看TCP%20IP协议/image-20241004120018946.png)


### 关闭TCP连接

由于我们没有使用keep-alive,当前HTTP请求结束后，client (PC0)会端口TCP连接，这一过程就是常说的四次挥手。这个过程在网上有详细的阐述，这里引用 [网上的材料](https://www.9tut.com/tcp-and-udp-tutorial)

![](assets/2024-09-09-从一次HTTP请求看TCP%20IP协议/image-20241004115520619.png)
